...
fn calculate_safety_status(
    ...
    return (!wolves_nearby && day_time) || has_carrot || friends_nearby > 3;
}
...
fn simulate(starting_rabbits: i128) -> i32 {
    ...
    if starting_rabbits % 2 == 1 {
        return simulate(starting_rabbits * 3 + 1) + 1;
    } else {
        return simulate(starting_rabbits / 2) + 1;
    }
}
